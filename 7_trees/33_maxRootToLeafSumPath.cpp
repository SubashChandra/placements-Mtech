...
void getPath(Treeptr root, vector<int> cur, vector<int> &ans,int curSum, int &maxSum) 
{
	if(root==NULL)
		return;
	...
	getPath(root->left,cur,ans,curSum,maxSum);
	getPath(root->right,cur,ans,curSum,maxSum);
}
...
void auxFunc(Treeptr root)
{
	...
	getPath(root,cur,ans,curSum,maxSum);
